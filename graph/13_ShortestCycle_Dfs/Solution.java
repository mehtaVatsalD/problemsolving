...
public class Solution {
	...
	private static int shortestCycle(int n, int[][] edges) {
		...
		for(int i=0; i<n; i++) {
			int minCycleLenVal = minCycleLen.isPresent() ? minCycleLen.get() : Integer.MAX_VALUE;
			if (v[i] == -1) {
				v[i] = 1;
	...
	private static Optional<Integer> shortestCycle(ArrayList<Integer>[] adj, int v[], int node, int parent, int index) {
		...
		for(int neigh: adj[node]) {
			int minCycleLenVal = minCycleAtThisNode.isPresent() ? minCycleAtThisNode.get() : Integer.MAX_VALUE;
			if (v[neigh] != -1) {
